...
import io.github.onedashboard.inspirationforge.mapper.ChatHistoryMapper;
...
public class ChatHistoryServiceImpl extends ServiceImpl<ChatHistoryMapper, ChatHistory> implements ChatHistoryService {
    ...
    public boolean addChatMessage(Long appId, String message, String messageType, Long userId) {
        ...
        App memoryApp = saved ? appService.getById(appId) : null;
        if (saved && memoryApp != null && Boolean.TRUE.equals(memoryApp.getRagEnabled())) {
            try {
                String role = ChatHistoryMessageTypeEnum.USER.getValue().equals(messageType) ? "用户需求" : "AI 结论";
                ragKnowledgeService.indexDocument(appId, userId, "CHAT",
                        "chat_history:" + chatHistory.getId(), role, role + "\n" + message,
                        "{\"messageType\":\"" + messageType + "\"}");
            } catch (Exception e) {
                log.warn("对话写入 RAG 长期记忆失败，不影响主流程, appId: {}", appId, e);
            }
        }
        return saved;
    }
