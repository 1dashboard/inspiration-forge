...
export interface VisualEditorOptions {
  ...
  onElementChanged?: (elementInfo: ElementInfo) => void
}
  ...
  handleIframeMessage(event: MessageEvent) {
    ...
    switch (type) {
      ...
      case 'ELEMENT_CHANGED':
        if (this.options.onElementChanged && data.elementInfo) {
          this.options.onElementChanged(data.elementInfo)
        }
        break
    }
  }
  ...
  private generateEditScript() {
    return `
      (function() {
        ...
        function notifyElementChanged(element) {
          try {
            window.parent.postMessage({
              type: 'ELEMENT_CHANGED',
              data: { elementInfo: getElementInfo(element) }
            }, '*');
          } catch {}
        }
        ...
        function addEventListeners() {
           ...
           document.body.addEventListener('blur', (event) => {
             if (!isEditMode || event.target !== currentSelectedElement) return;
             notifyElementChanged(event.target);
           }, true);
